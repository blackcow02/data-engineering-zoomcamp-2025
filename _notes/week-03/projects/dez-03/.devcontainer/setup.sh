...
# Create the database (if it doesn't exist)
psql -U "$POSTGRES_USER" -c "CREATE DATABASE $POSTGRES_DB_KESTRA;" || echo "Database already exists"
#psql -U "$POSTGRES_USER" -tc "SELECT 1 FROM pg_database WHERE datname = '$POSTGRES_DB_KESTRA'" | grep -q 1 || psql -U "$POSTGRES_USER" -c "CREATE DATABASE $POSTGRES_DB_KESTRA;"
